# Функция удаления заметок (изменено для выполненя задания)
...
def delete_note(notes):
    ...
    delete_position = input("Введите имя пользователя или заголовок для удаления заметки: ").strip().capitalize()
